...
def train(gen, disc, aux, gan, cfg):
    ...
    for epoch in range(cfg["epochs"]):
        ...
        with writer.as_default():
            tf.summary.scalar("disc_loss_real", np.mean(list_disc_loss_real), step=epoch)
            tf.summary.scalar("disc_loss_fake", np.mean(list_disc_loss_fake), step=epoch)
            tf.summary.scalar("disc_acc_real", np.mean(list_disc_acc_real), step=epoch)
            tf.summary.scalar("disc_acc_fake", np.mean(list_disc_acc_fake), step=epoch)
            tf.summary.scalar("gen_loss", np.mean(list_gen_loss), step=epoch)
            tf.summary.scalar("aux_loss_categorical", np.mean(list_aux_loss1), step=epoch)
            tf.summary.scalar("aux_loss_continuous1", np.mean(list_aux_loss2), step=epoch)
            tf.summary.scalar("aux_loss_continuous2", np.mean(list_aux_loss3), step=epoch)
